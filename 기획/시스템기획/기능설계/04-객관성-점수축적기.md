...
- 작성자: 이재혁
- 상태: 설계 확정, 구현 대기
- 관련: 일정-로드맵 §9(매체 객관성 동적 점수 / 빠른 안정화: 전량 record-only 스코어링),
...
> 실행 체크리스트 원본: [docs/superpowers/plans/2026-07-01-objectivity-scorer.md](../../../docs/superpowers/plans/2026-07-01-objectivity-scorer.md)

- **목표:** 요약하지 않은 수집 기사까지 감점 휴리스틱으로 채점해 매체별 객관성 점수를 이동평균(EWMA)으로 누적하는 observe-only 도구.
- **구조:** 독립 모듈 `src/objectivity.py`가 `raw/*.json`을 읽어 기사별 감점 채점 → 매체별 EWMA로 `scores/media.json`에 누적. curate/summarize와 격리(단방향 import만), daily 파이프라인에 미연결. CLI 또는 전용 스킬로 수동 실행.
- **구현 순서:** ① 기사별 감점 채점 `objectivity_score` → ② 매체 EWMA 집계 `update_media_scores`(같은 날짜 재처리 시 멱등) → ③ 저장 I/O + 날짜 기사 로딩(`curate.in_date_window` 재사용, 날짜창은 파일 날짜 기준) → ④ 하루 처리·백필·CLI 오케스트레이션 → ⑤ `scores/` git 추적 + 전용 스킬.
- **감점 사전:** 고정밀 시드만(AI_CONTEXT §6 피해야 할 표현 4개 + "~가 다 했네" 패턴). 단일 모호어 금지.
