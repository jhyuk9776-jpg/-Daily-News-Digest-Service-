...
run.sh (curate 뒤, 관망 훅):
```bash
echo "· (관망) 객관성 점수 축적"
python3 src/objectivity.py || true   # observe-only: 실패해도 다이제스트/자동커밋 안 막음
```

멱등성: `processed_dates`에 있는 날짜면 하루 채점을 건너뛴다. 워크플로우 중복 실행 시
...
- run.sh 관망 훅 연결, 파이프라인 정상(다이제스트 생성·자동커밋 무영향)
