...
매체별 객관성 점수를 이동평균(EWMA)으로 누적한다. observe-only — 선별·랭킹에 반영하지 않는다.
...
3. **결과 보고 — 두 가지를 같이 낸다:**
   - **당일 감점 메모**(`articles-<날짜>.json`의 `penalty_memo`): 총감점 · 표현별(근거)·횟수·감점 · 매체별 감점.
     "무엇이·왜·얼마나 깎였는지"를 사람이 읽게 정리한다.
   - **누적 표**(`media.json`): 매체별 점수(오름차순)·표본수·감점기사수.
