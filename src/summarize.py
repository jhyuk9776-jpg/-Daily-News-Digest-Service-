...
출력 형식: 사실 1~3개를 각각 한 줄 불릿("- ")으로만 출력한다. 다른 말은 하지 않는다."""
...
def run(date: str, dry_run: bool) -> int:
    ...
    for category, items in selected["categories"].items():
        for item in items:
            ...
            if content is None:
                results[link] = ([], "extract_failed")
                counters["extract_failed"] += 1
                print(f"  [추출실패] {category} / {item['title'][:30]}", file=sys.stderr)
                continue

            if link in cache:
                results[link] = (cache[link], "ok")
                counters["cached"] += 1
                counters["ok"] += 1
                continue

            if dry_run:
                results[link] = (["(dry-run: 요약 생략)"], "ok")
                counters["ok"] += 1
                continue

            try:
                bullets = summarize_one(item["title"], content["content_source"],
                                        content["text"])
                if not bullets:
                    raise ValueError("빈 요약")
                results[link] = (bullets, "ok")
                cache[link] = bullets
                counters["ok"] += 1
                print(f"  [요약] {category} / {item['title'][:30]} ({content['method']})")
            except Exception as exc:  # noqa: BLE001
                results[link] = ([], "api_failed")
                counters["api_failed"] += 1
                print(f"  [요약실패] {category} / {item['title'][:30]}: {exc}", file=sys.stderr)
